...
//        output
    ...
    public void LoadStockProducts() {
        try {
            System.out.println("Hearing Aid LoadStock Working");
//            aniwaren Login wenna wenawa
            ResultSet rs = MySQL.execute("SELECT * FROM `stock` INNER JOIN `product` ON `product`.`intid` = `stock`.`product_intid` INNER JOIN `sub_category` ON `sub_category`.`id` = `product`.`sub_category_id` INNER JOIN `category` ON `category`.`id` = `sub_category`.`category_id` INNER JOIN `brand` ON `brand`.`id` = `product`.`brand_id` INNER JOIN `location` ON `location`.`id` = `stock`.`location_id`  WHERE `category`.`id` =  '5'   AND `stock`.`location_id` = '" + UserDetails.UserLocation_id + "'  AND `qty` > 0 ");
    ...
    private void initComponents() {
        ...
        jLabel5.setText("Hearing Aid Orders");
        ...
        jPanel6.add(jLabel10, new org.netbeans.lib.awtextra.AbsoluteConstraints(770, 50, -1, -1));
        ...
        jPanel6.add(jTextField6, new org.netbeans.lib.awtextra.AbsoluteConstraints(410, 80, 280, -1));
        ...
        jPanel6.add(jScrollPane3, new org.netbeans.lib.awtextra.AbsoluteConstraints(410, 110, 450, 129));
        ...
        jPanel6.add(jTextField9, new org.netbeans.lib.awtextra.AbsoluteConstraints(770, 80, 90, -1));

        jLabel19.setFont(new java.awt.Font("Segoe UI Historic", 0, 14)); // NOI18N
        jLabel19.setText("Frame Id Or Brand");
        jPanel6.add(jLabel19, new org.netbeans.lib.awtextra.AbsoluteConstraints(410, 60, -1, -1));
...
//            System.out.println(String.valueOf(jTable3.getValueAt(jTable3.getSelectedRow(), 1)));
    ...
    private void jTextField6KeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTextField6KeyReleased
        ...
        try {
            //            aniwaren Login wenna wenawa
            ResultSet rs = MySQL.execute("SELECT * FROM `stock` INNER JOIN `product` ON `product`.`intid` = `stock`.`product_intid` INNER JOIN `sub_category` ON `sub_category`.`id` = `product`.`sub_category_id` INNER JOIN `category` ON `category`.`id` = `sub_category`.`category_id` INNER JOIN `brand` ON `brand`.`id` = `product`.`brand_id` INNER JOIN `location` ON `location`.`id` = `stock`.`location_id` "
                    + " WHERE `category`.`id` =  '5'  AND `qty` > 0  AND `stock`.`location_id` = '" + UserDetails.UserLocation_id + "' AND  `brand_name` LIKE '%" + brand_details + "%' OR `product`.`id` LIKE '%" + brand_details + "%' AND `category`.`id` =  '5' AND `stock`.`location_id` = '" + UserDetails.UserLocation_id + "' AND `qty` > 0  ");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(SellHearingAid.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new SellHearingAid().setVisible(true);
            }
        });
    }
